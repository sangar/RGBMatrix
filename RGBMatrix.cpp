...
extern const PROGMEM prog_char letters[];	//Get access to the 8X8 font to display letters on the matrix (located in RGB.h)	// added by me
...
char cRGBMatrix::fillChar(int screen, char letter, char color)
{
	...
	letter_offset = letter - '!'+1;	//Find the offset in the letters[] array
    letter_offset *=8;
    if(letter == ' ')letter_offset=0;	//If we get a space or invalid character, display a space
	if (!isascii(letter)) letter_offset=0;
	if (isspace(letter)) letter_offset=0;
	if (letter == '\0') letter_offset=0;
